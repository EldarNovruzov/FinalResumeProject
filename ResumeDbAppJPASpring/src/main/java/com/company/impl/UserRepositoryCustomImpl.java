...
//    @CacheEvict(value="users",allEntries = true)
    public boolean removeUser(int id) {
        ...
        Query query=em.createQuery("delete from User u where u.id=:uid");
        query.setParameter("uid", id);
        Country nationality = u.getNationality();
        Query q = em.createQuery("delete from Country c where c.id=:cid");
        q.setParameter("cid",nationality.getId());
        q.executeUpdate();
        query.executeUpdate();
        return true;
    }
    ...
    public List<User> getUserbyUserRoleId(Integer userRoleId) {
        String jpql="select u from User u where u.userRoleId.id=:role";
        Query q = em.createQuery(jpql, User.class);
        q.setParameter("role",userRoleId);
        return q.getResultList();
    }
...
//        return (String) crypt.hashToString(15,password);
//    }
//
//    public static void main(String[] args) {
//        String hash = (String) hash("1999fuad".toCharArray());
